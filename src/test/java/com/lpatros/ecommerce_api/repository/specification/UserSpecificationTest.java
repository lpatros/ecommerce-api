...
class UserSpecificationTest {
    ...
    void setUp() {
        ...
        doReturn(path).when(root).<Object>get("cpf");
        doReturn(path).when(root).<Object>get("name");
        doReturn(path).when(root).<Object>get("phoneNumber");
        doReturn(path).when(root).<Object>get("email");
    }

    @Test
    void filter_appliesAllPredicates_whenAllFiltersSet() {
        UserFilter filter = new UserFilter("123", "john", "119", "a@b.com");

        Specification<User> spec = UserSpecification.filter(filter);
        spec.toPredicate(root, query, cb);

        verify(cb).equal(any(), eq("123"));
        verify(cb).like(any(Expression.class), eq("%john%"));
        verify(cb).equal(any(), eq("119"));
        verify(cb).equal(any(), eq("a@b.com"));
    }
